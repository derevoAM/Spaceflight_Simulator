...
class RocketParameters:
    ...
    def __init__(self, initial_rocket_mass, fuel):
        self.parameters = np.array([7000e3, 0, 0, 7559.3])
...
class PhysicsEngine:
    ...
    def set_rocket_direction(self, angle):
        self.rocket_parameters.direction = rotate_vector(self.rocket_parameters.direction, angle)
    ...
    def calc_acceleration_engine(self, velocity_norm):
        ...
        if self.rocket_parameters.engine_is_on_flag and not self.rocket_parameters.is_empty():
            if velocity_norm > 1e-8:
                return self.constants.fuel_consumption * self.constants.gas_exhaust_speed / \
                       self.rocket_parameters.current_stage_mass * self.rocket_parameters.direction
            else:
                return np.array([
                    self.constants.fuel_consumption * self.constants.gas_exhaust_speed /
                    self.rocket_parameters.current_stage_mass, 0])
        else:
            return np.zeros(2)
    ...
    def calc_predicative_orbit(self):
        ...
        time_array = np.ndarray(shape=(log_size,), dtype=float)
...
if __name__ == "__main__":
    engine = PhysicsEngine(80000, 4000, 300, 50000, 50000)
